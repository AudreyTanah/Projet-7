...
function Carrousel({ pictures }) {//Props pictures dans LodgingPage.jsx
  ...
  function prevImg() {//Précédente image = prevImg
    ...
    if (nextIndex < 0) {//si on passe en dessous de 0         
  ...
  function nextImg() {
    ...
    if (nextIndex > pictures.length - 1) {//Si on est à la dernière l'image
      nextIndex = 0;//on repart au début du tableau
    }
    setImg({//Fonction changeant le state (initial) passe à l'image suivante avec la valeur de nextIndex déclarée avant
      src: pictures[nextIndex],
      index: nextIndex,
    });
  }
